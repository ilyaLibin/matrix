...
first_five_squares = { x**index for x,index in enumerate({1,2,3,4,5}, start=1 }



## 6: (Task 6) Powers-of-2 Set Comprehension
first_five_pows_two = { x**index for x,index in enumerate({1,2,3,4,5}, start=0 )}
...
LofL_sum = sum([sum(x) for x in [[.25, .75, .1], [-1, 0], [4, 4, 4, 4]] ]) 
...
odd_num_list_range = set(range(99))
...
range_and_zip = list(zip(['A','B','C','D','E'],range(5)))
...
list_sum_zip = [A[i]* B[i] for i in range(3)]
